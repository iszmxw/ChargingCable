...
class User extends Base
{
    ...
    public function agency_center()
    {
        ...
        $one_level      = "80";
        $one_level_free = "80";
        $one_level      = M("lc_apply")->where(['user_id' => $user_id])->value("one_level");
        $one_level_free = M("lc_apply")->where(['user_id' => $user_id])->value("one_level_free");
        if ($one_level == 0) {
            $one_level = M("lc_subcommission")->where(['id' => 1])->value("agent");
        }
        if ($one_level_free == 0) {
            $one_level_free = M("lc_subcommission")->where(['id' => 1])->value("agent_free");
        }
...
//        $data['code_id'] = $post['code_id'];
    ...
    public function apply_hotel()
    {
        ...
        $apply = M("lc_apply")->where(['user_id' => $this->user_id])->count();
        if ($apply > 0) {
            return returnBad('已有资料再审核中，请耐心等待！！');
        }
        if (empty($post['jd'])) {
            return returnBad('地理经度不能为空！');
        }
