...
class GoodsLogic {
    ...
    //获取商品可用优惠券列表
    public function getShopAvailableCoupon($cat_id,$user_id,$goods_id){
        $activityLogic = new ActivityLogic();
        $result = $activityLogic->getCouponCenterList($cat_id,$user_id, 1,$goods_id);
        foreach ($result as $key=>$value){
            $result[$key]['image'] = url_add_domain($value['image']);
        }
        return $result;
    }
