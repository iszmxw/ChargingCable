...
class UsersLogic extends Model
{
     ...
     * @param $openid
     */
    public function set_user_insert($user)
    {
        //$userinfo = db('users')->where('openid',$user['openid'])->field('openid,user_no,nickname,head_pic,level,pay_points')->find();
        $userinfo = db('users')->where('openid',$user['openid'])->value('openid');
        ...
        if (isset($user) && !$userinfo)
        {
            ...
            $userinfo = db('users')->where('user_id',$id)->value('openid');
        }
        //查询用户信息之后, 查询用户的登记昵称
        //$userinfo['level_name'] = Db::name("user_level")->where("level_id", $userinfo['level'])->getField("level_name");
    ...
    public function getApiUserInfo($user_id)
    {
        ...
        $user['finish']    = M('order')->where($order_where.C('FINISH'))->count(); //已完成数量
...
//            $comment_count   = M('order_goods')->where("order_id", $add['order_id'])->where('is_comment', 0)->count();
     ...
     * 维护卡明细
     */
    public function currency($user_id, $type='all')
    {
        if($type == 'all'){
            $count = M('account_log')->where("user_id=" . $user_id ." and pay_currency!=0 ")->count();
            $page = new Page($count, 16);
            $account_log = M('account_log')->where("user_id=" . $user_id." and pay_currency!=0 ")->order('log_id desc')->limit($page->firstRow . ',' . $page->listRows)->select();
        }else{
            $where = $type=='plus' ? " and pay_currency>0 " : " and pay_currency<0 ";
            $count = M('account_log')->where("user_id=" . $user_id.$where)->count();
            $page = new Page($count, 16);
            $account_log = M('account_log')->where("user_id=" . $user_id.$where)->order('log_id desc')->limit($page->firstRow . ',' . $page->listRows)->select();
        }
        foreach ($account_log as $val){
            ...
            switch ($val['desc']) {
                case '买入':
                    $val['pay_currency'] = '+' . $val['pay_currency'];
                    $val['desc'] = '转入';
                    $id_no = $val['seller_id'];
                    break;
                case '卖出':
            ...
            $user_no = db('users')->where('user_id',$id_no)->value('user_no');
            $val['user_no'] = $user_no?$user_no:'';
            $account_logs[]  = $val;
        }
        $result['account_log'] = $account_logs;
        $result['page'] = $page;

        return $result;
    }

    public function sell($user_id, $user_no, $pay_currency, $paypwd)
    {

        $user = Db::name('users')->where(['user_id'=>$user_id])->find();
        $user_sell = Db::name('users')->where(['user_no'=>$user_no])->find();
        //判断是否设置支付密码
        if (!$user['paypwd']){
            return ['status' => 309, 'msg' => "请先设置支付密码"];
            //return returnBad('请先设置支付密码','309');
        }
        //支付密码是否正确
        if ($user['paypwd'] != password_shal($paypwd)){
            return ['status' => 309, 'msg' => "您输入的密码不正确"];
            //return returnBad('您输入的密码不正确','309');
        }

        if ($user_no == $user['user_no']){
            return ['status' => -1, 'msg' => "自己不能转让"];
        }

        if (!$user_sell){
            return ['status' => -1, 'msg' => "买家ID错误"];
        }
        if ($user){
            ...
            try{
                //判断用户维护卡足够
                //if (bcsub($user['pay_currency'],200) < $pay_currency) return ['status' => -1, 'msg' => "用户维护卡不足"];
                if ($user['pay_currency'] < $pay_currency) return ['status' => -1, 'msg' => "用户维护卡不足"];

                $currency = bcsub($user['pay_currency'],$pay_currency);
                $currency_sell = bcadd($user_sell['pay_currency'],$pay_currency);

                $update = Db::name('users')->where(['user_id' => $user_id])->update(['pay_currency'=>$currency]);
                $update_sell = Db::name('users')->where(['user_no'=>$user_no])->update(['pay_currency'=>$currency_sell]);

                if ($update && $update_sell){
                    /* 插入帐户变动记录 */
                    $account_log = array(
                        array(
                            'user_id'       => $user_id,
                            'seller_id'       => $user_sell['user_id'],
                    ...
                    $res = Db::name('account_log')->insertAll($account_log);
            ...
            return ['status' => -101, 'msg' => "用户不在登录状态"];
        }
        //记录日志
        Log::write('插入帐户变动记录,' . $res);
        return $res;
    }
    ...
    public function addUserSign($user_id, $date)
    {
        ...
        if (Db::name('user_sign')->add($data)) {
            ...
            $result['sign_integral'] = $config['sign_integral'];
            //accountLog($user_id, 0, $config['sign_integral'], '第一次签到赠送' . $config['sign_integral'] . '积分');
            accountLog($user_id, 0, $config['sign_integral'], '签到赠送' . $config['sign_integral'] . '积分');
        }
        return $result;
    }
    ...
    public function updateUserSign($userInfo, $date)
    {
        ...
        if ($update>0) {
            ...
            if ($userFind['sign_count'] >= $config['sign_signcount']) {
                ...
                $result['msg']    = '哇，你已经连续签到' . $userFind['sign_count'] . '天！';
                $result['sign_integral'] = $config['sign_integral']+$config['sign_award'];
                $result['sign_count'] = $userFind['sign_count'];
    ...
    public function idenUserSign($user_id)
    {
        ...
        if(!$info){
            ...
            return ['info' => false, 'str' => "0,", 'jifen' => $jiFen, 'config' => $config,'display_sign'=>$display_sign];
        }
    ...
    public function checkUserQrcode($user){
        ...
        if(!$qrcode){
            $path="/pages/index/index?first_leader=".$user;
    ...
    public function son_user($user_id,$page = 1,$pagesize=10)
    {
        $data = Db::name('user_group')->where('user_id',$user_id)->value('a_son_id');
        if ($data){
            $info = explode(',',$data);
            foreach($info as $val){
                $user_info = db('users')
                    ->where('user_id',$val)
                    ->field('openid,nickname,level,head_pic,province,city')
                    ->find();
                switch ($user_info['level']){
                    case 1:
                        $user_info['level'] = '普通用户';
                        break;
                    case 2:
                        $user_info['level'] = '店长';
                        break;
                    case 3:
                        $user_info['level'] = '城市合伙人';
                        break;
                }
                $team[] =  $user_info;
            }

            $teams = pageReturn($team,$page,$pagesize);
        }


        return $teams ? $teams :['data'=>[],'total'=>0,'total_page'=>0,'current_page'=>0];
    }
     ...
     */
    public function share_qrcode_user($user)
    {

        $user_qrcode = Db::name('users')->field('nickname,xcx_qrcode')->where('user_id',$user)->find();

        if(empty($user_qrcode['xcx_qrcode'])){
            //$url = 'http://'.$_SERVER['SERVER_NAME'].'/index.php/api/Login/index?pid='.$user;
            $url = 'http://'.$_SERVER['HTTP_HOST'].'/index.php/api/Login/index?pid='.$user;
            //$url = 'http://'.$_SERVER['SERVER_NAME'].'/index.php/api/user/share_qrcode_user_binding?shareuser_id='.$user;
            //$url = 'https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=MzI4MDkwMzcwOA==#wechat_redirect';
            Log::write('打印url：'.$url);
            $user_qrcode['xcx_qrcode'] = distriCode($url,$user);
        }
        return $user_qrcode;
    }
     ...
     */
    public function real_mobile($user_id, $mobile)
    {
        ...
        if ($user_info)
        {
            $info = M('users')->where(['mobile'=>$mobile])->find();
            if ($info) return array('status'=>-1,'msg'=>'手机号码已存在','result'=>'');
            //更新数据
     ...
     * @throws \think\exception\DbException
     */
    public function real_name($user_id,$realname,$idcard,$address, $mobile)
    {
        if(!$realname && !$idcard && !$address && !$mobile){
            return array('status'=>400,'msg'=>'您还未实名','data'=>'');
    }
        //参数判断
        if (!preg_match('/^[\x7f-\xff]+$/', $realname))
        {
            return array('status'=>-1,'msg'=>'真实姓名格式有误','result'=>'');
        }
        //dump(mb_strlen($address,'UTF8'));die;
        if(!$address || (mb_strlen($address,'UTF8')<6))
            return array('status'=>-1,'msg'=>'地址长度必须6个字以上','result'=>'');
        if(!check_mobile($mobile) && !check_telephone($mobile))
            return array('status'=>-1,'msg'=>'手机号码格式有误','result'=>'');
        //更新数据
        ...
        if($user_info){
            ...
            return ['status'=>-1,'msg'=>'openid有误'];
        }
        return $res;
    }

    /**
     * 检查该用户是否存在维护卡买卖的专属二维码
     * @param $user 用户id
     * @return array|mixed|string
     */
    public function checkUserIdQrcode($user){
        $user_info = Db::name('users')->where('user_id',$user)->find();
        if($user_info){
            $path = $user_info['user_no'];
            $qrcode = distriCode($path);
        }else{
            return ['status'=>-1,'msg'=>'会员ID有误'];
        }
        return $qrcode;
    }
