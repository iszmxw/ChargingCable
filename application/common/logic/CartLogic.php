    ...
    public function buyNow()
    {
        ...
        if($this->goods['is_virtual'] == 1){
            if($this->goods['virtual_indate'] < time()){
                return ['status' => -1, 'msg' => '虚拟商品有效期已过', 'result' => ''];
            ...
            if (($this->goodsBuyNum + $isBuySum) > $this->goods['virtual_limit']) {
                return ['status' => -1, 'msg' => '您已超过该商品的限制购买数', 'result' => ''];
        ...
        if (empty($this->specGoodsPrice)) {
            ...
            if ($specGoodsPriceCount > 0) {
                ...
                //throw new TpshopException('立即购买', 0, );
            }
        ...
        if ($this->goodsBuyNum > $store_count) {
            return ['status' => -1, 'msg' => '商品库存不足，剩余' . $this->goods['store_count'], 'result' => ''];
        ...
        if ($goodsPromFactory->checkPromType($prom_type)) {
            ...
            if ($this->goods['prom_type'] == 0) {
                if (!empty($this->goods['price_ladder'])) {
                    ...
                    $buyGoods['member_goods_price'] = round($buyGoods['goods_price'] * $discount, 2);
                    $buyGoods['member_discount'] = $buyGoods['goods_price']-$buyGoods['member_goods_price'];
                    $buyGoods['goods_price'] = $buyGoods['member_goods_price'];
                }
            }
        }
        $cart = new Cart();
...
//        halt(($promGoodsLogic->checkActivityIsEnd() || !$promGoodsLogic->checkActivityIsAble()));
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public function getCartListBycartId($cart_ids = 0)
    {
        ...
        $cartWhere['id'] = ['in', $cart_ids];
        $cartWhere['combination_group_id'] = 0;
        $cartList = $cart->with('goods')->where($cartWhere)->select();  // 获取购物车商品
        if($cartList) {
            foreach ($cartList as $k => $v) {
                $cartList[$k]['shop_price'] = $v['goods_price'];
            }
        }
        $cartCheckAfterList = $this->checkCartList($cartList);
        return $cartCheckAfterList;
    ...
    public function changeNum($cart_id, $goods_num)
    {
        ...
        if(!$store_count){
            $store_count  = db('goods')->where(['goods_id'=>$cart['goods_id']])->value('store_count');
        }

        if ($goods_num + $cart_goods_num_sum > $store_count) {
            return ['status' => 0, 'msg' => $cart->goods_name.$cart->spec_key_name.'商品数量不能大于' . $store_count, 'result' => ['limit_num' => $store_count]];
        }
        if ($goods_num > 200) {
            $goods_num = 200;
        }
...
//            $carts = $Cart->where(['combination_group_id' => $cart['combination_group_id'], 'id' => ['neq', $cart['id']]])->select();
     ...
     * @throws \think\Exception
     */
    public function clearById($cart_ids)
    {
        Db::name('cart')->where(['user_id' => $this->user_id, 'id' => ["in", $cart_ids]])->delete();
    }
