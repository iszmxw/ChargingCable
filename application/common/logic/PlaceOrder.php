    ...
    public function addTeamOrder(Team $team)
    {
        ...
        if($teamFoundId){
            ...
            if($team_found_queue[$teamFoundId] <= 0){
                throw new TpshopException('提交订单', 0, ['status' => -1, 'msg' => '当前人数过多请耐心排队123!', 'result' => '']);
            }
...
//            }
    ...
    private function queueInc()
    {
        ...
        if(false/*$queue >= 100*/){
            throw new TpshopException('提交订单', 0, ['status' => -99, 'msg' => "当前人数过多请耐心排队456!", 'result' => '']);
        }
        Cache::inc('queue');
    }
    ...
    private function addOrderGoods()
    {
        ...
        foreach ($payList as $payKey => $payItem) {
            ...
            $orderGoodsData['shop_price'] = $payItem['shop_price']; // 不被折扣修改的商品价 商品实际价格
