...
//        }
		...
		if($data['type'] < 2){
            $data['goods_num'] = $data['goods_num'] ? $data['goods_num'] : $order_goods['goods_num'];
            ...
            //$data['refund_integral'] = $data['refund_integral']?$data['refund_integral']:$order_goods['refund_integral'];
	...
	function make_virtual_code($order){
        ...
        //给用户添加维护卡
        $pay_currency = M('users')->where(['user_id' => $order['user_id']])->value('pay_currency');
        $goods_num = $order_goods['goods_num'] * 100;
        $pay_currencys = $goods_num + $pay_currency;
        $saves = M('users')->where(['user_id' => $order['user_id']])->save(['pay_currency'=>$pay_currencys]);
        //插入帐户变动记录
        $account_log = array(
            'user_id'       => $order['user_id'],
            'pay_currency' => $goods_num,
            'pay_points'   => 0,
            'change_time'   => time(),
            'desc'   => "商城买入"
        );
        Db::name('account_log')->insert($account_log);
        file_put_contents('2.txt',$pay_currency.'---'.$pay_currencys.'---'.$saves);
