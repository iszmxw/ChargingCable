...
class GoodsLogic extends Model
{
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                //, 'href' => urldecode(U("Goods/$action", $filter_param, ''))
                $parr[] = array('value' => "{$end}元以下", "price" => "{$start}-{$end}");
            elseif($i == ($c-1) && ($max_price > $end))
                //, 'href' => urldecode(U("Goods/$action", $filter_param, ''))
                $parr[] = array('value' => "{$end}元以上", "price" => "{$start}-{$end}");
            else
                //, 'href' => urldecode(U("Goods/$action", $filter_param, ''))
                $parr[] = array('value' => "{$start}-{$end}元", "price" => "{$start}-{$end}");
        }
        return $parr;
    }
