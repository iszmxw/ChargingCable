...
function check_idCard($idCard){
    if(preg_match('/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/i',$idCard))
        return true;
    return false;
}
...
function url_add_domain($url){
    ...
    if(\think\Cache::has('app_domain')){
        $domain = \think\Cache::get('app_domain');
    }else{
 ...
 * @return array
 */
function returnBads($msg,$errorCode = 10000,$data=[]){
    header('Content-type: application/json');
    $result= json_encode(['code' => $errorCode,'msg'  => $msg,'data' =>$data]);
    echo $result;
    exit();
}
