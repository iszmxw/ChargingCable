...
class Report extends Base
{
  ...
  /**
    ...
    /**
     * [employ_analysis 使用分润统计]
     * @return [type] [description]
     */
    public function employ_analysis()  
    {
        $action = I('action');
        ...
        $Page = new Page($count,20);
        $group = empty($action) ? 'type' : $action ;
        ...
        $order_list = Db::name('can_commission')->where($where)->group($group)->field('SUM(price) as price,type,hotel_id,e_no')->select();

        $price = Db::name('can_commission')->where($where)->sum('price');

        foreach ($order_list as $key => $val) {

            if($group == 'type')
            {
                $name = $type[$val['type']-1];
            }else if($group == 'e_no') 
            {
                $name = $val['e_no'];
            }else
            {
                $name = $hotel[$val['hotel_id']];
            }
            $order_list[$key]['name'] = $name;
            $order_list[$key]['rate'] = bcdiv($val['price'],$price , 4) * 100;
        }
