...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
    ...
    public function agreement()
    {
        if(IS_POST){
            $data = input('post.');
            $agreement = M('system_article')->where(['doc_code'=>'agreement'])->find();
            if(empty($agreement)){
                $result = M('system_article')->add(['doc_code'=>'agreement','提现协议','doc_content'=>$data['doc_content']]);
            }else{
                $result = M('system_article')->where(['doc_code'=>'agreement'])->setField('doc_content',$data['doc_content']);
            }
            if($result!==false){
