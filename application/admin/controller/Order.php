...
//            $result = $payment_obj->payment_refund($data);
    ...
    public function add_return_goods()
   {
    ...
    public function refundWx()
    {
        ...
        $refundNo        = 'ORDERWX' . $order['user_id']. date('Ymd') . substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8) . rand(10000, 99000);
        $unified         = array(
            'appid'         => $config['appid'],
            'mch_id'        => $config['mch_id'],
            'nonce_str'     => self::createNonceStr(),
            'total_fee'     => intval($order['order_amount'] * 100),    //订单金额  单位 转为分
            'refund_fee'    => intval($order['order_amount'] * 100),    //退款金额 单位 转为分
            'sign_type'     => 'MD5',      //签名类型 支持HMAC-SHA256和MD5，默认为MD5
            //  'transaction_id'=>$wxOrderNo,        //微信订单号
            'out_trade_no'  => $order['order_sn'],    //商户订单号
            'out_refund_no' => $refundNo,    //商户退款单号
            'refund_desc'   => '不想要了！',   //退款原因（选填）
        );
        ...
        M("order")->where(['order_id' => $id])->save(['pay_status' => 3, 'admin' => $admins]);
